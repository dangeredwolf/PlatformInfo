export class PlatformInfo {
	...
	getOSVersion() {
		...
		switch(this.getOSPlatform()) {
			case "windows":
				...
				if (userAgent.match("Windows XP")) { // weird old browsers
					return "5.1";
				}
				return "4.0"; // Fallback, for really old browsers where NT version isn't reported
			case "windows9x":
	...
	isBrowserVersionLowerThan(version) {
		return !isBrowserVersionHigherThan(version);
	}

	isBrowserVersionHigherThan(version) {
		let testVersion = String(version).replace(/_/g, ".").split(".");
		let testVersionMajor = parseInt(testVersion[0]) || 0;
		let testVersionMinor = parseInt(testVersion[1]) || 0;

		let currentVersion = this.getBrowserVersion().split(".");
		let currentVersionMajor = parseInt(currentVersion[0]) || 0;
		let currentVersionMinor = parseInt(currentVersion[1]) || 0;
	...
	isOSVersionLowerThan(version) {
		return !this.isOSVersionHigherThan(version)
	}

	isOSVersionHigherThan(version) {
		let testVersion = String(version).replace(/_/g, ".").split(".");
		let testVersionMajor = parseInt(testVersion[0]) || 0;
		let testVersionMinor = parseInt(testVersion[1]) || 0;
		let testVersionPatch = parseInt(testVersion[2]) || 0;

		let currentVersion = this.getOSVersion().split(".");
		let currentVersionMajor = parseInt(currentVersion[0]) || 0;
		let currentVersionMinor = parseInt(currentVersion[1]) || 0;
		let currentVersionPatch = parseInt(currentVersion[2]) || 0;
